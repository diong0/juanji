...
class TensorBase(torch.nn.Module):  #继承torch的初始化
    def __init__(self, aabb, gridSize, device, density_n_comp=8, appearance_n_comp=24, app_dim=27,
        ...
        self.init_svd_volume(gridSize[0], device)  # 初始化特征网格

        self.shadingMode, self.pos_pe, self.view_pe, self.fea_pe, self.featureC = shadingMode, pos_pe, view_pe, fea_pe, featureC
        self.init_render_func(shadingMode, pos_pe, view_pe, fea_pe, featureC, device)

    def init_render_func(self, shadingMode, pos_pe, view_pe, fea_pe, featureC, device):  #指定渲染器
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1):
        ...
        # sample points
        viewdirs = rays_chunk[:, 3:6]  # 采样N个点，每个点带有6个信息，坐标xyz和表示光线朝向的位姿信息
        if ndc_ray:
        ...
        sigma = torch.zeros(xyz_sampled.shape[:-1], device=xyz_sampled.device)  #初始化体密度和颜色特征值张量
